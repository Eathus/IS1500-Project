#include "snake.h"

uint16_t SNAKES[128*4];

#define STRIPE_COLS 128
#define STRIPE_UNITS 8
#define UNIT_SIZE 2
#define STRIPE_ROWS 4

void insert_bit(uint8_t pos, uint8_t new_bit, uint16_t * ch){
    new_bit &= 0x01;
    uint16_t mask;
    if(new_bit){
        mask = new_bit << pos;
        *ch |= mask; 
    }
    else{
        mask = ((0xFE | new_bit) << pos) + ~(0xFF << pos);
        *ch &= mask;
    }
}

int unit_to_ustripe(Point coordinates){
    return (coordinates.x % STRIPE_COLS) * STRIPE_ROWS + (coordinates.y % 32) / STRIPE_UNITS;
} 
int get_ustripe_index(Point coordinates){
    return (coordinates.x % STRIPE_COLS) * STRIPE_ROWS + (coordinates.y % 4);
}
int get_unit_index(uint8_t y){
    return (y % 32) % STRIPE_UNITS;
}
uint16_t get_ustripe(Point coordinates, const uint16_t *snake){
    return snake[get_ustripe_index(coordinates)];
}

uint8_t get_unit(Point coordinates, const uint16_t *snake){
    uint16_t stripe = snake[unit_to_ustripe(coordinates)];
    return (stripe >> get_unit_index(coordinates.y) * UNIT_SIZE) & 0x0003;
}

void set_ustripe(Point coordinates, uint16_t val, uint16_t *snake){
    snake[get_ustripe_index(coordinates)] = val;
}

void set_unit(Point coordinates, uint8_t val, uint16_t *snake){
    uint8_t index = get_unit_index(coordinates.y) * UNIT_SIZE;
    insert_bit(index, val, snake + unit_to_ustripe(coordinates));
    insert_bit(index + 1, val >> 1, snake + unit_to_ustripe(coordinates));
}